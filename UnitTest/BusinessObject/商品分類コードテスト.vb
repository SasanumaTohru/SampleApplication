...
<TestClass()> Public Class 商品分類コードテスト
    ...
    <TestMethod()> <TestCategory("基本テスト")> Public Sub 商品分類コードテストの基本テスト()
        '分類コードは、1から始まる連番で採番される。分類コードは不変であり、削除されることもない。
        Dim 分類コード As New 分類コード(1)
        Assert.AreEqual(1, 分類コード.値)
    End Sub

    <TestMethod()> <TestCategory("例外テスト")> <ExpectedException(GetType(Exception))> Public Sub 商品分類クラス例外処理1()
        '商品分類に登録がないものはエラー。
        Dim 商品の分類 As New 分類コード(0)
    End Sub

    <TestMethod()> <TestCategory("例外テスト")> <ExpectedException(GetType(Exception))> Public Sub 商品分類クラス例外処理2()
        '商品分類に登録がないものはエラー。
